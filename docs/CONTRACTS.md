...
| `POST /api/roadmap/generate` | `{ hoursPerWeek?: number }` → NDJSON stream of `GenerationEvent`. The body is the pace the user chose; it is clamped to 1–60 and never rejected — absent, empty or unreadable falls back to 8, and it is written to `roadmaps.hours_per_week` alongside a local-clock `start_date`. Stages MUST be real work, personalized: `reading` (profile facts: n skills, named project), `comparing` (real count of cached postings for the target role), `gaps` (3 named gaps from assessments), `sequencing`. Final Gemini call returns 6–10 RoadmapTask drafts (each with why referencing profile specifics + posting requirements, `estimateHours`, cvLine, category, and its `steps`; task 1 `firstWeek: true`). `effort` is **derived, not generated** — `formatEffort(estimateHours)` from `lib/schedule.ts` writes the column, so the prose and the number the calendar is built from can never contradict each other. Persist roadmap → tasks → steps, all with the roadmap inactive, and only then flip actives (deactivate prior, activate this one); emit `done`. On failure mid-stream emit `error`. A crash between the two active statements leaves no active roadmap — `/roadmap` recovers by adopting the newest roadmap of the active target that has tasks and re-flagging it. |
...
| action `toggleStep` | `{ stepId, done }` → update one step of a task. Checking the LAST open step closes its task and takes the same `cv_versions` snapshot as `toggleTask`; un-checking one never reopens the task and never lowers the score. → `{ taskId, taskCompleted, score, delta }`. |
| action `replanFromToday` | `{ today?: string }` (`YYYY-MM-DD`, the caller's own local day) → rewrites `roadmaps.start_date` on **the roadmap `/roadmap` renders** (see the row below). No AI call: `lib/schedule.ts` re-flows the calendar on the next read. The day is validated as a real date and clamped to ±1 day of the server's local day (the widest honest timezone spread); omitted, it falls back to the server's local day. → `{ startDate }`. |
| action `changePace` | `{ hoursPerWeek }` → rewrites `roadmaps.hours_per_week` on that same roadmap, clamped 1–60 rather than rejected. No AI call — same re-flow as a re-plan, from the other input. → `{ hoursPerWeek }` (the value actually stored). |
| which roadmap the two date-only actions write to | `resolveRoadmapId` in `app/(app)/roadmap/actions.ts` resolves it by the rule `RoadmapPage` reads by, so the write side can always address the row the user is looking at: the roadmap flagged `active` **when its `target_id` is the target locked right now**, otherwise the newest roadmap of that target that has tasks (the same interrupted-flip recovery `/roadmap` performs — see the `POST /api/roadmap/generate` row). Matching on `active = true` alone would miss a recovered plan whose re-flag failed, and both controls would fail on a route the page had just drawn. With no active locked target, an active roadmap belonging to a *previous* target, or only zero-task rows to choose from, both actions fail with their usual message rather than writing to a row the page would not render — states only a stale open tab can reach, where failing is the correct answer. |
