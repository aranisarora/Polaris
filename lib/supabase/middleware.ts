...
/**
 * Gemini-backed endpoints under a per-user request budget. The shared free
 * tier is ~10 req/min (docs/CONTRACTS.md), so one user looping any of these
 * must not exhaust it for everyone. Budget: GEMINI_MAX_CALLS per
 * GEMINI_WINDOW_SECONDS — sized so the legitimate bearing burst (three
 * classify batches plus a concurrent dream assess) always fits.
 */
